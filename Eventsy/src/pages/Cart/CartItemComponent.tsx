...
// Define a union type for possible full details
export type FullServiceDetails = IFood | IVenue | IAnimator | IVehicle; // Export the type

interface CartItemComponentProps {
    item: CartItem;
    fullDetails?: FullServiceDetails;
    onRemove?: () => void;
    ...
    onDetailsClick: (item: CartItem, fullDetails?: FullServiceDetails) => void;
}
...
const CartItemComponent: FC<CartItemComponentProps> = ({ 
    ...
    const [editingClientDescription, setEditingClientDescription] = useState<{ id: string | null, value: string, isFocused: boolean }>({ id: null, value: item.clientDescription || '', isFocused: false });
                ...
                <div>
                    ...
                    {fullDetails && (
                        <div className="text-muted text-sm mt-1">
                            {'address' in fullDetails && <p>{fullDetails.address}</p>}
                            {'experience' in fullDetails && <p>Досвід: {fullDetails.experience} років</p>}
                            {'brand' in fullDetails && <p>Марка: {fullDetails.brand}</p>}
            ...
            <div className="space-y-4">
                {openDescriptionId === item.id && (
                    <div className="relative">
                        <textarea
                            ...
                            onBlur={(e) => {
                                onUpdateClientDescription?.(item.id, e.target.value);
                ...
                <div className="mt-4">
                    <button 
                         ...
                         className="w-full px-4 py-2 text-sm font-semibold text-white bg-coral rounded-md hover:bg-coral-dark transition-colors duration-200"
                     >
                         Детальніше
                     </button>
                 </div>
